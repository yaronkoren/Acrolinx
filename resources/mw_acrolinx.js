...
jQuery.fn.addAcrolinxAdapterToFormInput = function() {
	return this.each( function() {
		...
		if ( $(this).hasClass('tinymce') ) {
			var inputAdapter = new acrolinx.plugins.adapter.TinyMCEAdapter({editorId: textareaID});
		} else {
			var inputAdapter = new acrolinx.plugins.adapter.InputAdapter({editorId: textareaID});
		}
...
if ( action == 'edit' ) {
	var inputAdapter = new acrolinx.plugins.adapter.InputAdapter({editorId: 'wpTextbox1'});
	multiAdapter.addSingleAdapter(inputAdapter);
} else if ( action == 'tinymceedit' ) {
	var tinyMceAdapter = new acrolinx.plugins.adapter.TinyMCEAdapter({editorId: 'wpTextbox1'});
	...
	$('textarea').not('.multipleTemplateStarter textarea').addAcrolinxAdapterToFormInput();
}
...
$('div#acrolinxToggle').click(function() {
	if ( $(this).attr('minimized') == 'true' ) {
		...
		var acrolinxWidth = 300;
	} else {
		$(this).html( '<' );
		$(this).attr('minimized', 'true');
		var acrolinxWidth = 10;
	}
